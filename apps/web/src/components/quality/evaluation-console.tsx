...
export function EvaluationConsole({ agentId, evaluations }: EvaluationConsoleProps) {
  ...
  return (
    <div className="glass-card space-y-6 p-6">
      <div>
        <h2 className="text-lg font-headline text-ink">Evaluation Runner</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-3 rounded-lg border border-outline p-4">
        <div className="grid gap-3 md:grid-cols-2">
          <label className="text-xs uppercase tracking-wide text-ink-muted">
            Scenario name
            <input
              className="mt-1 w-full rounded-lg border border-outline bg-surfaceAlt/60 px-3 py-2 text-sm text-ink focus:border-brass/40 focus:outline-none"
              value={scenarioName}
      ...
      <div className="rounded-lg border border-outline p-4">
        <h3 className="text-sm font-semibold text-ink">Recent evaluations</h3>
        <ul className="mt-3 space-y-2 text-sm text-ink-muted">
          {latestEvaluations.length === 0 && (
            <li className="text-xs text-ink-muted">No evaluations captured yet.</li>
          )}
          {latestEvaluations.map((evaluation) => (
            <li key={evaluation.id} className="rounded-lg bg-surfaceAlt/60 p-3">
