...
export function BudgetControlsCard({ agentId }: BudgetControlsCardProps) {
  ...
  return (
    <Card className="h-full">
      <CardHeader>
        <CardTitle className="text-base font-semibold text-ink">Budget controls</CardTitle>
      ...
      <CardContent className="space-y-4">
        {isLoading || !data ? (
          ...
          <>
            ...
            <div className="flex items-center justify-between rounded-2xl border border-outline/50 bg-surfaceAlt/40 px-4 py-3">
              <div>
                <p className="text-sm font-medium text-ink">Auto reload credits</p>
                <p className="text-xs text-ink-muted">
            ...
            {mutation.isPending ? (
              <p className="text-xs text-ink-muted">Saving updates…</p>
            ) : null}
          </>
        )}
      </CardContent>
