    ...
    return (
      <Card className="border-white/70 bg-white/80">
        <CardContent className="p-6">
          <div className="h-12 w-full animate-pulse rounded-lg bg-outline/20" />
        </CardContent>
      </Card>
    );
  }
    ...
    <div className="space-y-6">
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full rounded-lg border border-outline/40 bg-surface px-3 py-2 text-sm text-ink placeholder:text-ink-muted/50"
              />
            </div>
      ...
      {account?.isOnboarded && (
        <Card className="border-white/70 bg-white/80">
          ...
          <CardContent>
            {history.length === 0 ? (
              <p className="text-sm text-ink-muted italic">No payouts yet.</p>
            ) : (
              <div className="space-y-3">
                {history.map((payout) => (
                  <div key={payout.id} className="flex items-center justify-between rounded-lg border border-outline/40 bg-surface/60 p-4">
                    <div className="flex-1">
                      ...
                      {payout.description && (
                        <p className="text-xs text-ink-muted">{payout.description}</p>
                      )}
                    </div>
                  </div>
                ))}
