  ...
  async createConnectedAccount(agentId: string, email: string) {
    ...
    try {
      ...
      const accountLink = await this.stripe.accountLinks.create({
        ...
        return_url: `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/console/billing`,
        refresh_url: `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/console/billing`,
      });
